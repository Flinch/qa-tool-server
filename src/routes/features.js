...
export async function patchFeature(req, res) {
  const { name, description } = req.body
  ...
  try {
    const { rows } = await query(
      `UPDATE features SET ${fields.join(', ')} WHERE id=$${i} RETURNING *`,
      values
    )
...
export async function deleteFeature(req, res) {
  try {
    const { rowCount } = await query(`DELETE FROM features WHERE id=$1`, [req.params.id])
