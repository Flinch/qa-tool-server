...
router.post('/runs/:runId/heal', ...staffOnlyChain, async (req, res) => {
  ...
  try {
    ...
    if (!['failed', 'skipped'].includes(result.status)) {
      return res.status(400).json({ error: 'Only a failed or skipped result can be healed' })
    }
...
router.post('/runs/:runId/diagnose', ...staffOnlyChain, async (req, res) => {
  ...
  try {
    ...
    const result = resultRows[0]
    if (!['failed', 'skipped'].includes(result.status)) {
      return res.status(400).json({ error: 'Only a failed or skipped result can be diagnosed' })
    }
  ...
  try {
    ...
    const { rows } = await req.db.query(`
      SELECT atc.id, atc.title, atc.origin, atc.review_status, atc.test_case_id,
        ...
        latest.report_url AS last_report_url, latest.github_run_url AS last_run_url
      FROM automated_test_cases atc
      JOIN automation_suites s ON s.id = atc.suite_id
      LEFT JOIN test_cases tc ON tc.id = atc.test_case_id
      LEFT JOIN LATERAL (
        SELECT trr.id, trr.test_run_id, trr.status, trr.error_message, trr.api_trace,
          tr.completed_at, tr.report_url, tr.github_run_url
        FROM test_run_results trr
        JOIN test_runs tr ON tr.id = trr.test_run_id
