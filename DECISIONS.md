...
## Phase 8 — platform segmentation (Web/Mobile) for test cases and requirements

- `test_cases.platform` and `requirements.platform` added (`web`/`mobile`, default `web`, additive) — closes a real gap where a web-only test case could be selected as a generation candidate for a mobile suite (or vice versa) with nothing stopping it, and where a requirement had no way to indicate which product surface it applies to.
- Deliberately a coarse 2-way enum, not a copy of `automation_suites.platform`'s 3-way `web/ios/android` — confirmed against real project 4 data that requirements 43-46 (Catalog browsing/Cart management/Authentication/Checkout) are each linked via `requirement_test_cases` to *both* an Android test case (75-77) and an iOS test case (78-80). A strict 3-way requirement platform would force each of these to pick one OS and break that real data; 2-way matches how requirements are actually used (one business requirement, tested separately per mobile OS via distinct test-case rows, same convention already in place before this change). Within "mobile," iOS-vs-Android distinction stays exactly as it already was: separate test-case rows, not a schema column.
- One-off backfill (not in `migrate.js` — same category as other one-off real-data seeds, not idempotent schema): `test_cases` 75,76,77,78,79,80 → `platform='mobile'`; `requirements` 43,44,45,46 → `platform='mobile'`. Everything else defaulted to `'web'`, which is correct for every other existing row (this initiative is the only mobile work in the tool so far).
- `triggerGenerationRun` (`automationTrigger.js`) now validates candidate test cases' `platform` matches the target suite's category (`web` suite → `web` TCs, `ios`/`android` suite → `mobile` TCs) server-side, not just via client-side filtering — same "never trust the client alone" principle as the existing `automation_candidate` check it sits next to.
- `POST /:reqId/test-cases` (manual requirement↔test-case linking) gained the same server-side platform gate.
- Known, accepted residual gap: coarse filtering means an iOS-titled test case can still appear as a candidate when generating for the *Android* suite specifically (and vice versa) — not closed here, since the generation flow's confirm-step already shows full TC titles before dispatch, making this a visible, catchable mistake rather than a silent one. A nullable finer column is a clean future addition if this ever becomes a real problem in practice.
